...
try:
    ...
    gpio_available = True
    print("GPIO 12 initialized LOW - Pi in setup mode, not ready for PIC commands")
except Exception as e:
    print(f"GPIO setup failed: {e}")                                    

def set_pi_ready_state(ready=True):
    """Set GPIO 12 to signal PIC about Pi readiness
    ready=True: GPIO HIGH (Pi ready for commands)
    ready=False: GPIO LOW (Pi busy, don't send commands)
    """
    if gpio_available:
        try:
            GPIO.output(12, GPIO.HIGH if ready else GPIO.LOW)
            state = "READY" if ready else "BUSY"
            print(f"GPIO 12 set to signal Pi is {state}")
        except Exception as e:
...
class BatchScannerApp:
    ...
    def _on_manual_scan_timeout(self):
        ...
        if self.awaiting_hardware:
            ...
            self._complete_controller_request("SKIP")                        
    ...
    def stop_scanning(self, show_message=True):
        ...
        try:
            ...
            if is_legacy_mode():
                ...
                legacy.send_firmware_command('E')                     
